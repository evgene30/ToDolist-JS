...
function delMessage(buttonClick) { // функция удаления сообщений
  buttonClick.querySelector('.del_button')
    .addEventListener('click', (event) => {
      const { parentElement } = event.target.parentElement; //  находим родительский блок для удаления
      const itemIndex = parentElement.getAttribute('id');
...
function activeClick() {
  ...
  const listItems = document.querySelectorAll('.main-list__items li'); // находим все элементы списка
  const impButtom = document.querySelectorAll('.mark-list__item'); // находим кнопки
